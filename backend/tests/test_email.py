...
from src.shared.email import get_email_config, send_reset_password_email
from src.auth.models import User


@pytest.fixture
def mock_settings(mocker):
    mock = mocker.patch("src.shared.email.settings")
...
async def test_send_reset_password_email(mock_settings, mock_user, mocker):
    # Mock FastMail
    mock_fastmail = mocker.patch("src.shared.email.FastMail")
    ...
    assert [recipient.email for recipient in message_arg.recipients] == [
        mock_user.email
    ]
